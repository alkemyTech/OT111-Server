...
public class UserAuthServiceImpl implements UserAuthService {
    ...
    public AuthenticationResponse loginAttempt(AuthenticationRequest authenticationRequest) throws Exception {
        ...
        try {
            ...
            SecurityContextHolder.getContext().setAuthentication(newTry);//mantiene iniciada la secion -
        } catch (BadCredentialsException e) {
            throw new Exception("Incorrect username or password", e);
        }
        // Build Response:
        String jwt = jwtTokenUtil.generateToken(userDetails);//se genera token con jwt(con los valores que necesite)
